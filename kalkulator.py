...
# =======================================================
# 🚀 PROCALC PREMIUM OFFER VIEW - LINK DLA KLIENTA
# =======================================================
...
if "oferta" in query_params:
    ...
    def _zbierz_materialy(dane):
        materialy = []

        if dane.get("zbiorcza_lista_zakupow"):
            materialy = dane.get("zbiorcza_lista_zakupow", [])
        elif dane.get("materialy_lista"):
            materialy = dane.get("materialy_lista", [])
        elif dane.get("etapy"):
            for etap in dane.get("etapy", []):
                for mat in etap.get("materialy_lista", []):
                    materialy.append(mat)
    ...
    try:
        ...
        for i, etap in enumerate(etapy):
            nazwa_etapu = etap.get("nazwa_etapu", etap.get("branza", f"Etap {i + 1}"))
            branza_etapu = etap.get("branza", "Zakres prac")
            koszt_etapu = etap.get("koszt_robocizny", etap.get("koszt_calkowity", 0))

            etapy_html += f"""
            <tr>
                <td>
                    <div class="stage-name">{_safe_html(nazwa_etapu)}</div>
                    <div class="stage-sub">{_safe_html(branza_etapu)}</div>
                </td>
                <td>{_money_html(koszt_etapu)}</td>
            </tr>
            """
        ...
        if materialy:
            for mat in materialy:
                ...
                <div class="material-row">
                    <span>{_safe_html(nazwa_mat)}</span>
                    <strong>{_safe_html(wartosc)}</strong>
                </div>
                """
        else:
            materialy_html = """
...
<style>
    ...
    .premium-offer {{
        ...
        border-radius: 18px;
        overflow: hidden;
        box-shadow: 0 24px 70px rgba(8, 40, 73, 0.12);
    }}

    .offer-hero {{
        min-height: 360px;
        background:
            linear-gradient(90deg, rgba(4, 32, 62, .96), rgba(4, 32, 62, .70), rgba(4, 32, 62, .20)),
            url('{hero_uri}');
    ...
    .offer-footer {{
        ...
        padding: 26px 34px;
        display: grid;
        grid-template-columns: 220px 1fr 90px 170px;
        align-items: center;
        gap: 24px;
    }}

    .offer-footer img.logo-footer {{
        max-width: 180px;
    }}
...
<div class="premium-offer">
    ...
    <section class="offer-hero">
        ...
        <div class="hero-meta">
            <div class="hero-pill">
                ...
                <strong>{data_wystawienia}</strong>
            </div>
            <div class="hero-pill">
                <span>Projekt</span>
                <strong>{_safe_html(nazwa_klienta)}</strong>
            </div>
            <div class="hero-pill">
                <span>Status</span>
                <strong>{_safe_html(status_badge)}</strong>
            </div>
        </div>
    </section>
    ...
    <section class="offer-body">
        <div>
            ...
            <div class="info-grid">
                ...
                <div class="info-box">
                    <span>Forma rozliczenia</span>
                    <strong>Robocizna / etapy</strong>
                </div>
            </div>

            <h2 class="section-title">Podsumowanie oferty</h2>

            <div class="summary-cards">
                <div class="summary-card">
                    <span>Suma robocizny</span>
                    <strong>{_money_html(suma_rob)}</strong>
                </div>
                <div class="summary-card red">
                    <span>Udzielony rabat</span>
                    <strong>-{_money_html(rabat)}</strong>
                </div>
                <div class="summary-card green">
                    <span>Łącznie do zapłaty</span>
                    <strong>{_money_html(do_zaplaty)}</strong>
                </div>
            </div>

            <div class="accept-box">
                <div class="status-badge">{_safe_html(status_badge)}</div>
                <p style="margin:0;color:#42566f;font-size:14px;">
                    Po akceptacji wykonawca otrzyma informację, że oferta została zatwierdzona.
                </p>
            </div>
        </div>
    ...
    <section class="materials-section">
        <div class="material-box">
            <div class="material-head">Logistyka i zapotrzebowanie materiałowe</div>
            {materialy_html}
        </div>

        <div class="terms-box">
            <div class="material-head">Notatki i warunki współpracy</div>
            <div class="terms-content">
                <p>Oferta obejmuje zakres prac wskazany w zestawieniu kosztów.</p>
                ...
                <p>Płatność: zaliczka 30%, pozostała część po zakończeniu lub według ustalonego harmonogramu.</p>
                <p>Końcowy koszt może ulec zmianie w przypadku zmiany zakresu prac przez inwestora.</p>
            </div>
        </div>
    </section>
...
""")
        ...
        if status == "Podpisano":
            ...
            if projekt.get("podpis_imie_nazwisko"):
                st.info(f"Podpisujący: {projekt.get('podpis_imie_nazwisko')}")
            ...
            st.info("Podpisz palcem na telefonie albo myszką na komputerze.")
            ...
            canvas_result = st_canvas(
                ...
                key=f"canvas_podpis_{oferta_id}",
            )

            zgoda_podpis = st.checkbox(
                "Potwierdzam akceptację zakresu prac, ceny oraz warunków współpracy.",
                key=f"zgoda_podpis_{oferta_id}"
            )
            ...
            if st.button("Zatwierdź podpis i zakończ akceptację", type="primary", use_container_width=True):
                ...
                elif not zgoda_podpis:
                    st.error("Zaznacz potwierdzenie akceptacji warunków.")
                elif canvas_result.image_data is None:
                    st.error("Złóż podpis w polu powyżej.")
                else:
                    try:
                        img = canvas_result.image_data

                        # Sprawdzamy, czy podpis nie jest pusty.
                        has_signature = (img[:, :, :3] < 245).any()

                        if not has_signature:
                            st.error("Podpis wygląda na pusty. Złóż podpis w polu powyżej.")
                        else:
                            from PIL import Image

                            podpis_img = Image.fromarray(img.astype("uint8"), "RGBA")
                            bufor = BytesIO()
                            podpis_img.save(bufor, format="PNG")
            ...
            col_accept, col_negotiate, col_reject = st.columns(3)
            ...
            with col_negotiate:
                with st.popover("Zaproponuj cenę", use_container_width=True):
                    propozycja = st.number_input(
                        "Twoja propozycja ceny brutto",
                        min_value=0.0,
                        value=float(do_zaplaty),
                        step=100.0,
                        key="propozycja_klienta"
                    )

                    komentarz = st.text_area(
                        "Komentarz dla wykonawcy",
                        placeholder="Np. Czy możemy zejść do tej kwoty przy tym samym zakresie?",
                        key="komentarz_negocjacji"
                    )
        ...
        with st.container(border=True):
            ...
            if prace_dodatkowe_pdf:
                ...
                for i, praca in enumerate(prace_dodatkowe_pdf):
                    cpd_a, cpd_b = st.columns([4, 1])
                    with cpd_a:
                        st.write(
                            f"- **{praca.get('nazwa')}** | robocizna: {praca.get('robocizna', 0):,.2f} zł | materiały: {praca.get('materialy', 0):,.2f} zł".replace(",", " ")
                        )
                    with cpd_b:
            ...
            dane_pdf_oferta = {
                **_dane_firmy_pdf(),
                ...
                "etapy": etapy_pdf,
                "prace_dodatkowe": prace_dodatkowe_pdf,
                "klauzule": klauzule_pdf,
    ...
    try:
        ...
        if not projekty:
            ...
            for p in projekty:
                ...
                # --- PANEL NEGOCJACJI DLA WYKONAWCY ---
                if status == "Negocjacja":
                    propozycja_klienta = p.get("propozycja_klienta")
                    propozycja_wykonawcy = p.get("propozycja_wykonawcy")
                    historia = p.get("historia_negocjacji") or []

                    with st.container(border=True):
                        st.subheader(f"Negocjacja: {nazwa}")

                        if propozycja_klienta:
                            st.warning(
                                f"Klient zaproponował cenę: **{float(propozycja_klienta):,.2f} zł**".replace(",", " ")
                            )

                        if propozycja_wykonawcy:
                            st.info(
                                f"Twoja ostatnia kontrpropozycja: **{float(propozycja_wykonawcy):,.2f} zł**".replace(",", " ")
                            )

                        col_neg1, col_neg2, col_neg3 = st.columns(3)

                        with col_neg1:
                            if st.button("Akceptuj propozycję klienta", key=f"accept_neg_{projekt_id}", type="primary", use_container_width=True):
                                if propozycja_klienta:
                                    try:
                                        nowa_historia = historia + [{
                                            "typ": "akceptacja_propozycji_klienta",
                                        ...
                                        st.success("Propozycja klienta zaakceptowana. Klient może teraz zaakceptować ofertę po nowej cenie.")
                                        time.sleep(1)
                                        st.rerun()
                                    except Exception as e:
                                        st.error(f"Błąd akceptacji propozycji: {e}")
                                else:
                                    st.error("Brak aktywnej propozycji klienta.")

                        with col_neg2:
                            if st.button("Odrzuć propozycję", key=f"reject_neg_{projekt_id}", use_container_width=True):
                                try:
                                    nowa_historia = historia + [{
                                        "typ": "odrzucenie_propozycji_klienta",
                                        "kwota": float(propozycja_klienta) if propozycja_klienta else None,
                                        "data": datetime.now().isoformat()
                                    }]
                                    ...
                                    st.info("Propozycja została odrzucona. Oferta wraca do aktualnej ceny.")
                                    time.sleep(1)
                                    st.rerun()
                                except Exception as e:
                                    st.error(f"Błąd odrzucenia propozycji: {e}")

                        with col_neg3:
                            with st.popover("Zaproponuj nową cenę", use_container_width=True):
                                obecna_kwota = p.get("kwota_aktualna") or kwota
                                kontr_kwota = st.number_input(
                                    "Twoja kontrpropozycja",
                                    min_value=0.0,
                                    value=float(obecna_kwota),
                                    step=100.0,
                                    key=f"kontr_kwota_{projekt_id}"
                                )

                                komentarz_wykonawcy = st.text_area(
                                    "Komentarz do klienta",
                                    placeholder="Np. Mogę zejść do tej ceny przy płatności zaliczki 30%.",
                                    key=f"kontr_komentarz_{projekt_id}"
                                )

                                if st.button("Wyślij kontrpropozycję", key=f"send_kontr_{projekt_id}", use_container_width=True):
                                    try:
                                        nowa_historia = historia + [{
                                            "typ": "propozycja_wykonawcy",
                                            "kwota": float(kontr_kwota),
                                            "komentarz": komentarz_wykonawcy,
                                            "data": datetime.now().isoformat()
                                        }]
                                        ...
                                        st.success("Kontrpropozycja została wysłana. Klient zobaczy nową cenę w linku oferty.")
                                        time.sleep(1)
                                        st.rerun()
                                    except Exception as e:
...
# --- PRZYCISK ANULOWANIA EDYCJI ---
    ...
    # --- SEKCJA: EFEKTY DEKORACYJNE ---
        ...
        if not st.session_state.koszyk_projektow:
            ...
            for i, etap in enumerate(st.session_state.koszyk_projektow):
                ...
                with st.expander(f"Etap {i+1}: {etap['nazwa_etapu']} | {etap['branza']} | {kwota_etapu} zł", expanded=True):
                    c1, c2 = st.columns(2)
           ...
           # 2. PODSUMOWANIE FINANSOWE CAŁOŚCI
            ...
            # --- NOWOŚĆ: RABAT TYLKO NA ROBOCIZNĘ ---
            st.markdown("### Negocjacje (Rabat na robociznę)")
            st.info("Zniżka odliczana jest tylko od kwoty za pracę. W ofercie dla klienta pokażemy tylko koszty robocizny.")
            
            rabat_kwota = st.number_input(
                "Udziel rabatu od robocizny (PLN):", 
                min_value=0.0, 
                max_value=float(suma_robocizna), # Zabezpieczenie: rabat max do kwoty robocizny
                step=50.0, 
                value=0.0
            )
            
            robocizna_po_rabacie = suma_robocizna - rabat_kwota
            
            if rabat_kwota > 0:
                st.success(f"Wartość robocizny po rabacie: **{robocizna_po_rabacie:,.2f} zł**".replace(",", " "))
                ...
                if sprawdz_dostep_pro():
                    ...
                    if st.button("ZAPISZ CAŁY KOSZTORYS DO CHMURY", type="primary", use_container_width=True):
                        ...
                        if not nazwa_glownego_projektu.strip():
                            ...
                            # Tutaj tworzymy zaktualizowany słownik (tylko na robociznę)
                            dane_do_bazy = {
                                "koszt_calkowity_projektu": suma_calkowita, # Zapisujemy dla Twojej wiedzy wewnętrznej
                                "suma_robocizna": suma_robocizna,
                                "suma_materialy": suma_materialy,
                                "rabat_kwota": rabat_kwota,
                                "robocizna_po_rabacie": robocizna_po_rabacie,
                                "ukryj_ceny_materialow": True, # Flaga informująca PDF, żeby usunął ceny materiałów!
        ...
        if not st.session_state.zalogowany:
            ...
            with tab_tynki:
                ...
                if "GK" in metoda_tynku:
                    ...
                    liczba_plyt = math.ceil(pow_scian_gk / 3.12 * 1.05)
                    worki_kleju_gk = math.ceil(pow_scian_gk / 4)

                    st.success(
                        f"Potrzeba: **{liczba_plyt} płyt** oraz "
                        f"**{worki_kleju_gk} worków** kleju."
                    )
                else:
                    st.info(
                        f"Tynk maszynowy zostanie policzony dla "
                        f"**{pow_scian_gk} m2** ścian."
                    )
    ...
    # ==========================================
        ...
        widok_harmonogramu = st.pills(
            "Widok harmonogramu:",
            ["Harmonogram", "Kalendarz", "Tydzień", "Miesiąc", "Lista"],
            selection_mode="single",
            default="Harmonogram",
            key="widok_harmonogramu"
        )
        ...
        if widok_harmonogramu == "Kalendarz":
            st.subheader("Kalendarz prac")
        
            zadania_sort = sorted(zadania, key=lambda x: x.get("data_start", ""))
        
            for z in zadania_sort:
                data_s = _parse_data_harm(z.get("data_start"))
                data_k = _parse_data_harm(z.get("data_koniec"))
        
                with st.container(border=True):
                    c1, c2, c3 = st.columns([1.4, 2.4, 1.2])
        
                    with c1:
                        st.markdown(f"### {data_s.strftime('%d.%m')} - {data_k.strftime('%d.%m')}")
                        st.caption(f"{(data_k - data_s).days + 1} dni")
        
                    with c2:
                        st.markdown(f"**{z.get('nazwa_projektu', 'Projekt')}**")
                        st.write(z.get("nazwa_etapu", "Etap"))
                        if z.get("lokalizacja"):
                            st.caption(z.get("lokalizacja"))
        
                    with c3:
                        _status_badge(z.get("status", "Nie rozpoczęto"))
        ...
        if widok_harmonogramu == "Tydzień":
            ...
            koniec_tyg = start_tyg + timedelta(days=6)
        
            st.caption(f"{start_tyg.strftime('%d.%m.%Y')} - {koniec_tyg.strftime('%d.%m.%Y')}")
        
            zadania_tyg = []
            for z in zadania:
                s = _parse_data_harm(z.get("data_start"))
                k = _parse_data_harm(z.get("data_koniec"))
        
                if s <= koniec_tyg and k >= start_tyg:
                    zadania_tyg.append(z)
        
            if not zadania_tyg:
                st.info("Brak prac zaplanowanych na ten tydzień.")
                st.stop()
        
            for i in range(7):
                dzien = start_tyg + timedelta(days=i)
                prace_dnia = []
        
                for z in zadania_tyg:
                    s = _parse_data_harm(z.get("data_start"))
                    k = _parse_data_harm(z.get("data_koniec"))
                ...
                with st.container(border=True):
                    ...
                    if not prace_dnia:
                        ...
                        for z in prace_dnia:
                            st.write(f"**{z.get('nazwa_etapu')}** - {z.get('nazwa_projektu')}")
                            if z.get("lokalizacja"):
                                st.caption(z.get("lokalizacja"))
        
            st.stop()

        if widok_harmonogramu == "Miesiąc":
            st.subheader("Podsumowanie miesiąca")
        
            dzis = date.today()
            start_mies = dzis.replace(day=1)
        
            if dzis.month == 12:
                next_month = dzis.replace(year=dzis.year + 1, month=1, day=1)
            else:
                next_month = dzis.replace(month=dzis.month + 1, day=1)
        
            koniec_mies = next_month - timedelta(days=1)
            ...
            for z in zadania:
                ...
                if s <= koniec_mies and k >= start_mies:
                    zadania_mies.append(z)
        
                    d = max(s, start_mies)
                    while d <= min(k, koniec_mies):
                        dni_zajete.add(d)
                        d += timedelta(days=1)
        
            m1, m2, m3 = st.columns(3)
            m1.metric("Zlecenia w miesiącu", len(zadania_mies))
            m2.metric("Dni zajęte", len(dni_zajete))
            m3.metric("Dni wolne", max(0, (koniec_mies - start_mies).days + 1 - len(dni_zajete)))
        
            st.markdown("---")
        
            if not zadania_mies:
                st.info("Brak prac w tym miesiącu.")
                st.stop()
        
            for z in sorted(zadania_mies, key=lambda x: x.get("data_start", "")):
                with st.container(border=True):
        ...
        if widok_harmonogramu == "Harmonogram":
            ...
            wysokosc_harmonogramu = 260 + (len(projekty) * 92)
            components.html(harmonogram_html, height=max(620, wysokosc_harmonogramu), scrolling=True)
